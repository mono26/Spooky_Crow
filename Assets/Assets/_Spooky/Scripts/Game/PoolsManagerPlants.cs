...
public class PoolsManagerPlants : MonoBehaviour {
    ...
    private void AddObject(int index)      //Se le pasa el index que debe de ser igual para le pool y el objeto
    {
        ...
        obj.GetComponent<AIPlantController>().my_PlantInfo.index = index;       //Modifica el index del componente del objeto instanciado.
    ...
    public void ReleaseObject(GameObject obj)     ////Se le pasa el index que debe de ser igual para le pool y el objeto
    {
        ...
        obj.gameObject.SetActive(false);
        pools[obj.GetComponent<AIPlantController>().my_PlantInfo.index].Add(obj);
    }
}
